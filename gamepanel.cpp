...
#include <QMouseEvent>
#include <QPainter>
#include <QRandomGenerator>//生成随机数的类
#include <QTimer>
#include <QDebug>
    ...
    , ui(new Ui::GamePanel)
{
    ...
    //设置背景图，有十张图，利用随机数，随机选择一张
    int num = QRandomGenerator::global()->bounded(10);//随机0-9
    QString path = QString(":/images/background-%1.png").arg(num+1);//利用Qstring拼接出图片的路径
    m_bkImage.load(path);
...
void GamePanel::gameControlInit()
{
    ...
    m_playerList << leftRobot << rightRobot << user;//将三个对象放入vector中， ，这里的 << 插入操作
...
void GamePanel::initCardMap()//初始化卡牌(初始化card类，并为其添加上和cardpanel的映射关系)
{
    //加载大图
...
{
    //每裁剪出一张图，便创建一个cardpanel对象，将图片赋给cardpanel
    ...
    //初始化时不需要显示出来，因此先将其隐藏
    panel->hide();
    //将card对象和cardpanel对象存入m_cardMap
...
void GamePanel::initPlayerContext()//初始化玩家上下文
{
    //初始化 1，放置玩家扑克牌的区域
    const QRect cardsRect[] =
        {
            ...
            QRect(250, rect().bottom() - 120, width() - 500, 100)//人类玩家玩家
            //以下都按，左，右，人类的顺序初始化的
        };
    //2，玩家出牌的区域
    ...
    //遍历玩家数组，将玩家和玩家上下文信息加入 m_contextMap中
    int index = m_playerList.indexOf(m_gameCtl->getUserPlayer());//获取人类玩家
    for(int i=0; i<m_playerList.size(); ++i)
    {

        PlayerContext context;
        context.align = i==index ? Horizontal : Vertical;//根据玩家类型，设置对齐方式
        ...
        context.info->hide();//一开始先隐藏
        //设置提示信息的显示区域，假设出牌区域中心点为(x,y)，则提示信息中心位置为(x/2,y/2)
        ...
        //玩家上下文信息初始化完毕，将玩家及信息加入到 m_contextMap中
