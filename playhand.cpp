...
{
    //1，先对扑克牌进行分类，例如：1张，2张，3张，4张的各有多少种
    classify(cards);
    //2，对排序进行分类
    judgeCardType();
}
...
{
    //先对所有牌做一个桶排序，记录每个点数有多少张牌
    CardList list = cards.toCardList();//Cardlist为类型别名，实际上是vector<card>类型
    int cardRecord[Card::Card_End];
    memset(cardRecord, 0, sizeof(int) * Card::Card_End);//初始化以下数组
    ...
    for(int i=0; i<Card::Card_End; ++i)
    {
        if(cardRecord[i] == 1)
        {
            m_oneCard.push_back((Card::CardPoint)i);//需要做一个强制类型转换
        }
...
bool PlayHand::canBeat(const PlayHand &other)
{
    ...
    {
        if(m_type == Hand_Seq_Pair || m_type == Hand_Seq_Single)//如果为顺子
        {
            //比较点数和顺子的个数
...
bool PlayHand::isPlane()
{
    if(m_oneCard.isEmpty() && m_twoCard.isEmpty() && m_threeCard.size() == 2 && m_fourCard.isEmpty())
    {
        ...
        if(m_threeCard[1] - m_threeCard[0] == 1 && m_threeCard[1] < Card::Card_2)//两组三张的牌点数相差1
        {
            return true;
        }
    }
...
bool PlayHand::isPlaneTwoSingle()
{
    if(m_oneCard.size() == 2 && m_twoCard.isEmpty() && m_threeCard.size() == 2 && m_fourCard.isEmpty())
    {
        ...
        if(m_threeCard[1] - m_threeCard[0] == 1 && m_threeCard[1] < Card::Card_2 &&
            m_oneCard[0] != Card::Card_SJ && m_oneCard[1] != Card::Card_BJ)//带的两张单拍不能为大小王
        {
            return true;
        }
    }
